...
import {
  getApoderadoDashboard,
  getDetalleAlumnoApoderado
} from '../api/bffApi.js';
...
import {
  generarCertificadoAlumnoRegular
} from '../utils/certificadoAlumnoRegular.js';
...
export default function Apoderado() {
  ...
  const [
    certificadoMensaje,
    setCertificadoMensaje
  ] = useState('');

  const [
    generandoCertificado,
    setGenerandoCertificado
  ] = useState(false);
  ...
  const cargarDatosApoderado =
    async () => {
      ...
      try {
        ...
        setError(
          e.message ||
          'Error al cargar el panel del apoderado.'
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleAlumnoChange = async (
    ...
    try {
      const detalle =
        await getDetalleAlumnoApoderado(
          user.id,
          alumnoId
        );
      ...
      setError(
        e.message ||
        'Error al cargar detalle del alumno.'
      );
    }
  };
  ...
  const solicitarCertificado = () => {
    ...
    try {
      ...
      setCertificadoMensaje(
        'El certificado de alumno regular fue generado correctamente.'
      );
    } catch (e) {
      setError(
        e.message ||
  ...
  return (
    <div className="role-page">
      ...
      {alumnos.length === 0 && (
        <div className="role-card">
          ...
          <p className="text-muted">
            No hay estudiantes asociados a
            este apoderado.
          </p>
        </div>
      )}
      ...
      {selectedAlumno && (
        <>
          <div className="student-info-card student-info-card-simple">
          ...
          {activeTab ===
            ...
            <div className="role-card">
              ...
              <div className="certificate-option-card">
                <div>
              ...
              <div className="payment-option-card mt-4">
                <div>
                  <span className="payment-label">
                    Pago arancel
                  </span>
                ...
                <button
                  className="btn-base btn-secondary-custom"
                  type="button"
                  onClick={() =>
                    setPagoMensaje(
                      'El pago con Webpay será habilitado próximamente.'
                    )
                  }
                >
                  Pagar con Webpay
