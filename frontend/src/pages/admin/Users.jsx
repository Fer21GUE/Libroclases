      ...
      <div className="content-section">
        ...
        <div className="table-responsive">
          <table className="table table-dark table-striped">
            <thead><tr><th>ID</th><th>Nombre</th><th>Email</th><th>Rol</th><th>Activo</th><th>Habilitar o inhabilitar usuarios</th></tr></thead>
            <tbody>{users.map(u => <tr key={u.id}><td>{u.id}</td><td>{u.nombre}</td><td>{u.email}</td><td>{u.rol}</td><td>{u.activo ? 'Sí' : 'No'}</td><td><div className="form-check form-switch"><input className="form-check-input" type="checkbox" role="switch" checked={!!u.activo} onChange={() => onToggleActivo(u.id)} /><label className="form-check-label">{u.activo ? 'Habilitado' : 'Inhabilitado'}</label></div></td></tr>)}</tbody>
          </table>
        </div>
      </div>
    </AdminPageLayout>
