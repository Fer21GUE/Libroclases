...
export default function Users() {
  ...
  return (
    <AdminPageLayout title="Gestión de usuarios">
      ...
      <div className="content-section"><h2>Listado de usuarios</h2><div className="table-responsive"><table className="table table-dark table-striped"><thead><tr><th>ID</th><th>Nombre</th><th>Email</th><th>Rol</th><th>Activo</th></tr></thead><tbody>{users.map(u => <tr key={u.id}><td>{u.id}</td><td>{u.nombre}</td><td>{u.email}</td><td>{u.rol}</td><td>{u.activo ? 'Sí' : 'No'}</td></tr>)}</tbody></table></div></div>
    </AdminPageLayout>
  );
}
